...
describe('Api Routes', () => {
    describe('Placeholders routes', () => {
        it('Get Images', testRouteApplication(3032, '127.0.0.1', apiRoutes, { method: 'get', path: '/api/images' }, response => {
        ...
        it('Get Categories', testRouteApplication(3033, '127.0.0.1', apiRoutes, { method: 'get', path: '/api/categories' }, response => {
            const list = response.data
            expect(list.length).toBe(1)
            expect(list[0].name).toBe('Gallery')
        }))
        it('Get Settings', testRouteApplication(3034, '127.0.0.1', apiRoutes, { method: 'get', path: '/api/settings' }, response => {
            const list = response.data
